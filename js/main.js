(function($){
    ...
    // }
    new ScrollMagic.Scene({triggerElement: "#trigger0", duration: "100%"})
    .on("enter", function (event) {
            var i = parseInt($('#trigger0').attr('id').slice(-1));
            swiper.slideTo(i+1);
            //$('.scroller').removeClass('is-visible');
    }).triggerHook(0.8).addIndicators({name: "trigger slide 1"}).addTo(controller);

            setTimeout(function() { 
                //alert($("#trigger1").parent().height());

                new ScrollMagic.Scene({triggerElement: "#trigger1", duration: $("#trigger1").parent().height()})
                    .on("enter", function (event) {
                        ...
                        //alert('exit');
                        mainTl.pause();
                    }).triggerHook(0.8).addIndicators({name: "trigger slide 2"}).addTo(controller);

                // new ScrollMagic.Scene({triggerElement: "#trigger2", duration: $("#trigger2").parent().height()})
                //     .on("enter", function (event) {
                ...
                new ScrollMagic.Scene({triggerElement: "#trigger10"})
                    .on("enter", function (event) {
                        ...
                        swiper.slideTo(i+1);
                    }).triggerHook(0.8).addIndicators({name: "trigger footer"}).addTo(controller);
